...
export default function AdminPlansPage() {
  ...
  async function load() {
    setLoading(true);
    const t = setTimeout(() => setLoading(false), 4000); // هرطور شده از لود خارج شو
    try {
      // دید ادمین: اگر ویو v_plans_admin داری از اون بخون؛ اگر نه، از plans بخون
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        <div style={{ display: "grid", gap: 10, gridTemplateColumns: "1fr 1fr" }}>
          ...
          <input
            ...
            onChange={(e) =>
              setForm((p) => ({ ...p, profit_percent: e.target.value }))
            }
          />
          <input
            placeholder="مدت (روز)"
        ...
        <div style={{ marginTop: 12, display: "flex", gap: 8 }}>
          ...
          {form.id && (
            <button className="nv-btn" onClick={reset}>
              انصراف
            </button>
          )}
        </div>
              ...
              >
                <div>
                  ...
                  <div className="muted" style={{ marginTop: 4 }}>
                    سود: {r.profit_percent}% — مدت: {r.duration_days} روز — حداقل:{" "}
                ...
                <div style={{ display: "flex", gap: 8 }}>
                  ...
                  <button className="nv-btn" onClick={() => remove(r.id)}>
                    حذف
                  </button>
                </div>
              </div>
