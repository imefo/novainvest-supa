    ...
    <div className="nv-container" style={{ paddingTop: 24 }}>
      ...
      <div className="card" style={{ marginBottom: 16 }}>
        <div style={{ display: "grid", gap: 10, gridTemplateColumns: "1fr 1fr" }}>
          ...
          <input placeholder="درصد سود" type="number" value={form.profit_percent}
                 onChange={(e) => setForm((p) => ({ ...p, profit_percent: e.target.value }))} />
          <input placeholder="مدت (روز)" type="number" value={form.duration_days}
                 onChange={(e) => setForm((p) => ({ ...p, duration_days: e.target.value }))} />
          <input placeholder="حداقل سرمایه" type="number" value={form.min_amount}
          ...
          <textarea style={{ gridColumn: "1 / -1" }} placeholder="توضیحات"
      ...
      <div className="card">
        {loading ? (
          ...
          <div style={{ display: "grid", gap: 10 }}>
            {rows.map((r) => (
              <div key={r.id} className="card" style={{ display: "grid", gridTemplateColumns: "1fr auto", alignItems: "center" }}>
                <div>
                  <strong>{r.name}</strong>{" "}
                  <span className="muted">({r.type})</span>
                  <div className="muted" style={{ marginTop: 4 }}>
                    سود: {r.profit_percent}% — مدت: {r.duration_days} روز — حداقل: {Number(r.min_amount || 0).toLocaleString()} — {r.is_active ? "فعال" : "غیرفعال"}
                  </div>
                </div>
